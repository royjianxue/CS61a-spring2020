...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, beehive):
        ...
        #start at current place. Keep looking for bees until the hive. If we find a spot with a bee (or mutiple bees), return one of them
        current_pos = self.place
        steps = 0
        while current_pos != beehive:
            if current_pos.bees and steps >= self.min_range and steps <= self.max_range:
                return random_or_none(current_pos.bees)
            steps += 1
            current_pos = current_pos.entrance
        return None
        # END Problem 3 and 4
...
class FireAnt(Ant):
    ...
    def reduce_armor(self, amount):
        ...
        "*** YOUR CODE HERE ***"
        self.armor -= amount
        # Ant.reduce_armor(self, amount) <- cannot use this because it may remove any immediately


        # reducing all bees' armor by the same amount of FireAnt
        for bee in self.place.bees[:]:
            bee.reduce_armor(amount)

        #if FireAnt's armor is zero or less, 1. if there are bees, reduce the bees' armor by damage then remove the bee and send the callback
        if self.armor <= 0:
