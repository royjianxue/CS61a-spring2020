...
def scheme_eval(expr, env, _=None): # Optional third argument is ignored
    ...
    if scheme_symbolp(first) and first in SPECIAL_FORMS:
        ...
        #using built in map funciton from Pair Class
...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(target): 
        ...
        # BEGIN PROBLEM 5
        "*** YOUR CODE HERE ***"
        #1.evaluate the variable's value in the given env
         #a. which part of expressions corresponds to the value of the variable?
         #b. what function do we already have that evaluates for us? 
        #2. Then, we need to define the evaluated value to the variable name
         #a. Take a look at the Frame class (you've already written this function!)
        #3. Finally we want to return the name of the variuable we just define
        expr = expressions.rest.first  #second element 
        value = scheme_eval(expr, env)
        value.define(target, value)
        return target
